...
/// actually shown here is [entityImageProvider]'s in-memory-cropped
/// rendering of that raw image, using whichever `ClippingSlot` is
/// currently registered for [slot] under the app's one active clipping
/// profile (see `../../pages/clipping_settings.dart` for where that
/// profile itself is configured/switched — never here, since picking a
/// raw image for one individual must not change which crop template
/// every other individual renders through); since that provider reacts
/// to both the raw image and the current clipping profile, registering
/// or changing a crop template — or switching profiles — is reflected
/// immediately, for every individual, without re-picking anything.
