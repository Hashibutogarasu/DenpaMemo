...
void main() {
  test('search() resolves an antenna id to its cached anntenaCategory and finds the matching column', () async {
    ...
    final categoryCacheRepository = EvasionRateCategoryCacheRepository(objectBox);
    ...
    final metadataCacheRepository = PhysiqueTableMetadataCacheRepository(objectBox);
    ...
    final service = PhysiqueIdentificationService(
      ...
      antennaCategoryResolver: PhysiqueAntennaCategoryResolver(metadataCacheRepository),
      awaitInitialSync: () async {},
    );

    final result = await service.search(hp: 32, evasionRate: 0, level: '1', antenna: 'none');

    expect(result.matches, [
      isA<PhysiqueColumnMatch>()
          .having((match) => match.level, 'level', '1')
          .having((match) => match.anntenaCategory, 'anntenaCategory', 'アンテナ無し')
