...
void main() {
  ...
  test('ring grants +3 charm only', () {
    final result = abnormalityByAttribute(build(headShapeById('ring')));
    expect(result, {'charm': 3});
  });

  test('sun grants +3 to every regular abnormality but not jack', () {
  ...
  test('silkHat grants +2 to every regular abnormality and +1 jack', () {
    final result = abnormalityByAttribute(build(headShapeById('silkHat')));
    expect(result['jack'], 1);
    expect(
      result.entries.where((e) => e.key != 'jack').map((e) => e.value).toSet(),
      {2},
    );
  });

  test('bowlCut only grants +1 jack', () {
    final result = abnormalityByAttribute(build(headShapeById('bowlCut')));
    expect(result, {'jack': 1});
  });

  test('castleTower grants +2 jack only', () {
    final result = abnormalityByAttribute(build(headShapeById('castleTower')));
    expect(result, {'jack': 2});
  });

  test('grandCastleTower grants +3 jack only', () {
    final result = abnormalityByAttribute(
      build(headShapeById('grandCastleTower')),
    );
    expect(result, {'jack': 3});
  });

  test('frog grants +1 to every regular abnormality and +1 jack', () {
